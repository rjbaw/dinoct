...
- Supports label files formatted as either:
  ...
  - `N×2` table `(x, z)` (the second column is used; `x` is preserved on save).
- Mouse editing:
  - Left click/drag: set `z` at the nearest `x` sample.
  - Right click: set interpolation anchor (same as pressing `i` once).
- Keyboard controls to move along x and adjust z, interpolate, smooth, save, undo/redo, and navigate samples.

Assumptions
- Labels represent pixel coordinates in the image coordinate system (origin top-left; z increases downward).
- The number of z-values may differ from image width; labels are displayed over the image by resampling x positions.
- Image coordinate system: origin at top-left, z increases downward (matches PIL/imshow upper origin).
...
- up/down: adjust z at selected x by 1 (Shift=±5, Ctrl=±10).
...
def read_labels(path: Path) -> LabelData:
    ...
    if arr.shape[1] == 2:
        ...
        z = arr[:, 1].astype(np.float64, copy=False)
    elif arr.shape[1] == 1:
        z = arr[:, 0].astype(np.float64, copy=False)
    elif arr.shape[0] == 1:
    ...
    return LabelData(z=z, x=x)


def write_labels(path: Path, labels: LabelData) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    if labels.x is None:
        np.savetxt(path, labels.z, fmt="%.6f")
    else:
        xy = np.stack([labels.x, labels.z], axis=1)
...
class CurveEditor:
    ...
    def load_current(self):
        ...
        self.labels_saved = LabelData(
            z=self.labels.z.copy(), x=(self.labels.x.copy() if self.labels.x is not None else None)
        )
        n = int(self.labels.z.shape[0])
        ...
        # Clamp z to image bounds
        self.labels.z = np.clip(self.labels.z, 0, self.height - 1)
    ...
    def is_dirty(self) -> bool:
        if self.labels is None or self.labels_saved is None:
            return False
        return not np.allclose(self.labels.z, self.labels_saved.z)

    def push_undo(self):
        assert self.labels is not None
        self.undo_stack.append(self.labels.z.copy())
    ...
    def interpolate_between(self, i0: int, i1: int):
        ...
        if i0 > i1:
            i0, i1 = i1, i0
        self.push_undo()
        y0, y1 = float(self.labels.z[i0]), float(self.labels.z[i1])
        k = i1 - i0
        if k <= 1:
            return
        self.labels.z[i0 : i1 + 1] = np.linspace(y0, y1, k + 1)
    ...
    def smooth_at(self, center_idx: int):
        ...
        i1 = min(len(self.labels.z) - 1, center_idx + half)
        seg = self.labels.z[i0 : i1 + 1]
        k = int(min(len(seg), w))
        if k <= 1:
            return
        ...
        self.push_undo()
        self.labels.z[i0 : i1 + 1] = smoothed[: len(seg)]
    ...
    def adjust_y(self, delta: float):
        ...
        assert self.height is not None
        self.push_undo()
        self.labels.z[self.cur_xi] = float(np.clip(self.labels.z[self.cur_xi] + delta, 0, self.height - 1))
    ...
    def status(self) -> str:
        ...
        msg = (
            ...
            f"x[{self.cur_xi}]={self.x_positions[self.cur_xi]:.1f}  z={self.labels.z[self.cur_xi]:.1f}  step={self.step:.2f}  smooth_w={self.smooth_w}  autosave={autosave}"
        )
    ...
    def redraw(self):
        ...
        self.line_artist.set_data(self.x_positions, self.labels.z)
        self.marker_artist.set_data([self.x_positions[self.cur_xi]], [self.labels.z[self.cur_xi]])
        if self.anchor is None:
            self.anchor_artist.set_data([], [])
        else:
            self.anchor_artist.set_data([self.x_positions[self.anchor]], [self.labels.z[self.anchor]])
    ...
    def on_key(self, event):
        ...
        if key == "end":
            assert self.labels is not None
            self.cur_xi = len(self.labels.z) - 1
            self.redraw()
            return
        if key.endswith("up"):
        ...
        if key == "u":
            if self.undo_stack:
                assert self.labels is not None
                self.redo_stack.append(self.labels.z.copy())
                self.labels.z = self.undo_stack.pop()
                self.toast = "undo"
                self._quit_armed = False
                self.redraw()
            return
        if key == "U":
            if self.redo_stack:
                assert self.labels is not None
                self.undo_stack.append(self.labels.z.copy())
                self.labels.z = self.redo_stack.pop()
                self.toast = "redo"
                self._quit_armed = False
                self.redraw()
            return
    ...
    def _apply_mouse_edit(self, x: float, z: float) -> None:
        assert self.labels is not None
        assert self.width is not None and self.height is not None
        ...
        self.cur_xi = idx
        self.labels.z[idx] = float(np.clip(z, 0, self.height - 1))
        self._quit_armed = False
        self.redraw()
...
def main():
    ...
    parser.add_argument("--step", type=float, default=1.0, help="Base z-step in pixels for up/down")
