...
def load_backbone_from_pretrain(ckpt_path: Path, device: torch.device) -> tuple[torch.nn.Module, int]:
    ...
    if not isinstance(state, dict):
        raise ValueError(f"Unsupported pretrain checkpoint format: {type(state)}")
    # torch.compile saves params under a "_orig_mod." prefix; the backbone here is built
    # uncompiled, so strip it or every key silently fails to match and the backbone stays
    # at random init.
    state = {k.removeprefix("_orig_mod."): v for k, v in state.items()}
    result = backbone.load_state_dict(state, strict=False)
    if result.missing_keys or result.unexpected_keys:
        raise RuntimeError(
            f"Pretrain checkpoint/backbone key mismatch for arch={arch!r}: "
            f"missing={result.missing_keys[:8]} unexpected={result.unexpected_keys[:8]} "
            "— refusing to visualize a partially initialized backbone."
        )
    backbone.eval()
...
def load_curve_model(
    *,
    ...
    result = model.load_state_dict(remap_legacy_curve_head_keys(state), strict=False)
    if result.missing_keys or result.unexpected_keys:
        raise RuntimeError(
            f"Curve checkpoint/model key mismatch (backbone={backbone_name!r}, "
            f"lora_blocks={lora_blocks}): missing={result.missing_keys[:8]} "
            f"unexpected={result.unexpected_keys[:8]} — refusing to visualize a partially "
            "initialized model; for an un-fused checkpoint pass --lora-blocks/--lora-r/"
            "--lora-use-mlp matching the training config."
        )
    model.eval()
    return model
...
def _draw_curve_rgb(
    base_rgb: np.ndarray, z_vec: np.ndarray, *, color: tuple[int, int, int] = (255, 0, 0), radius: int = 1
) -> np.ndarray:
    out = base_rgb.copy()
    H, W = out.shape[0], out.shape[1]
    if z_vec.shape != (W,):
        raise ValueError(f"Expected z_vec shape ({W},), got {z_vec.shape}")
    xs = np.arange(W, dtype=np.int64)
    zs = np.clip(np.round(z_vec).astype(np.int64), 0, H - 1)
    for off in range(-radius, radius + 1):
        zz = zs + off
        m = (zz >= 0) & (zz < H)
        out[zz[m], xs[m]] = np.array(color, dtype=np.uint8)
    return out
...
def infer_lora_from_state_dict(state: dict[str, Any]) -> tuple[int, int]:
...
def prepare_curve_model(args: argparse.Namespace, *, device: torch.device) -> tuple[CurveModel, transforms.Compose]:
    ...
    if inferred_patch is not None:
        patch_size = int(inferred_patch)
    elif backbone_name.startswith("convnext"):
        # ConvNeXt's logical token grid is set by config, not weights, so it cannot be
        # inferred from the checkpoint and must match the training config's
        # student.patch_size (14 in the dinoct training convention, the default here).
        patch_size = int(args.patch_size)
    else:
        patch_size = native_patch_size_for_backbone(backbone_name, int(args.patch_size))

    inferred_blocks, inferred_r = infer_lora_from_state_dict(state)
    lora_blocks = inferred_blocks if args.lora_blocks is None else int(args.lora_blocks)
    lora_r = inferred_r if args.lora_r is None else int(args.lora_r)
    print(
        f"curve model: backbone={backbone_name} patch_size={patch_size} "
        f"lora_blocks={lora_blocks} lora_r={lora_r}"
    )
...
def run_curve_image(
    *,
    ...
    with torch.inference_mode():
        ...
        z_vec = soft_argmax_height(curve_logits[:, :-1, :]).detach().cpu().float().numpy().reshape(-1)
        prob_hw = torch.softmax(curve_logits, dim=1)[0].detach().cpu().float().numpy()

    writer.path("curve_presence.txt").write_text(f"{p_curve:.6f}\n")
    np.savetxt(writer.path("curve_z.csv"), z_vec[None, :], delimiter=",", fmt="%.6f")

    base_rgb = np.array(img_resized, dtype=np.uint8)
    overlay = _draw_curve_rgb(base_rgb, z_vec, radius=2)
...
def prepare_backbone(args: argparse.Namespace, *, device: torch.device) -> tuple[torch.nn.Module, int]:
    ...
    if ckpt_path is not None and ckpt_path.exists():
        ...
        print(
            "WARNING: no backbone checkpoint found; feature maps will use a randomly "
            "initialized backbone. Pass --backbone-ckpt for meaningful features.",
            file=sys.stderr,
        )
    ...
    *,
    ...
    if seed_s in {"auto", "cls"}:
        ...
        try:
            ...
            raise ValueError(f"Invalid --seed '{seed}'. Use 'auto', 'center', or 'x,z' pixels.") from exc
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--seed",
        default="auto",
        help="Seed patch for seed_similarity_*: 'auto'|'center'|'x,z' in pixels of the eval crop",
    )
    parser.add_argument(
        "--save-attn-heads",
        action="store_true",
    ...
    parser.add_argument("--lora-blocks", type=int, default=None, help="Default: inferred from the checkpoint")
    parser.add_argument("--lora-r", type=int, default=None, help="Default: inferred from the checkpoint")
