...
def _assert_oct_shape(image: np.ndarray, *, relpath: str | Path) -> None:
    if tuple(image.shape) != EXPECTED_OCT_SHAPE:
        raise ValueError(
            "Expected OCT image {} to have shape {}, got {}. "
            "Classical baselines are fixed-parameter 512x500 implementations; "
            "resize or retune explicitly before evaluating a different image size.".format(
                relpath,
                EXPECTED_OCT_SHAPE,
                tuple(image.shape),
            )
...
def _evaluate_method(
    *,
    ...
    for idx, entry in enumerate(entries):
        ...
        _assert_oct_shape(raw, relpath=filename)
        ...
        if background is not None:
            _assert_oct_shape(background, relpath=background_relpath)
        ...
        z_true = dataset.get_target(int(idx))
        if z_true is None:
            continue
        t0 = time.perf_counter()
        z_pred = detect_surface_curve(method_key, raw, background=background)
        runtime_ms = (time.perf_counter() - t0) * 1000.0
        metrics = curve_metrics_batch(
            np.asarray(z_pred[None, :], dtype=np.float32),
            np.asarray(z_true, dtype=np.float32)[None, :],
        ...
        if write_overlays and overlays_written < max(int(overlay_limit), 0):
            ...
            _draw_curve_overlay(
                raw,
                np.asarray(z_pred, dtype=np.float32),
