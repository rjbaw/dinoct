...
def _predict_curve_z_hat(
    ...
    *,
    ...
    if is_segmentation_baseline_model_type(resolved_model_type):
        ...
        z_hat, _ = decode_paper_unet_logits(seg_logits)
        if tta_hflip:
            seg_logits_flip = model(images.flip(dims=(-1,)), orig_hw=(ORIG_H, ORIG_W))
            z_hat_flip, _ = decode_paper_unet_logits(seg_logits_flip)
