...
from dinoct.data import DataAugmentationDINO, MaskingGenerator, collate_data_and_cast, make_data_loader, make_dataset
from dinoct.data.datasets import OCT
from dinoct.layers import DINOHead
from dinoct.loss import DINOLoss, KoLeoLoss, iBOTPatchLoss
from dinoct.models.vision_transformer import vit_small
from dinoct.train.core.schedules import cosine_schedule, linear_warmup_cosine_decay
from dinoct.train.param_groups import fuse_params_groups, get_params_groups_with_decay_fsdp
from dinoct.utils import fix_random_seeds

from convnext_backbone_norm_ablation import (
    convnext_small,
    convnext_small_s2,
    convnext_tiny,
    convnext_tiny_s2,
)
from post_train_backbone_norm_ablation import run_post_training
    ...
    try:
        ...
        if hasattr(torch, "_dynamo") and hasattr(torch._dynamo, "config"):
            try:
                ...
                pass
        # Passing backend=None to torch.compile wraps successfully but raises
        # BackendCompilerFailed at the first forward (outside this try/except), so omit
        # the kwarg entirely when no backend is configured and let torch pick its default.
        if backend is None:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--post-train-bg-weight", type=float, default=None)
    ...
    parser.add_argument(
        ...
        choices=["none", "final_norm_only", "all_norms"],
        default=None,
        help="Which backbone normalization layers remain trainable during post-train.",
    )
    parser.add_argument(
        "--post-train-backbone-checkpointing",
        action="store_true",
        help="Enable activation checkpointing through the backbone during post-train.",
    )
    parser.add_argument(
        "--post-train-method",
        choices=["sam", "adamw"],
...
def main() -> None:
    ...
    if post_steps and post_steps > 0:
        ...
        post_bg_weight = args.post_train_bg_weight
        post_lr_warmup = (
        ...
        run_post_training(
            ...
            bg_weight=float(post_bg_weight if post_bg_weight is not None else post_cfg.get("bg_weight", 5.0)),
            ...
            backbone_norm_mode=str(args.post_train_backbone_norm_mode or post_cfg.get("backbone_norm_mode", "final_norm_only")),
            backbone_checkpointing=bool(args.post_train_backbone_checkpointing or post_cfg.get("backbone_checkpointing", False)),
