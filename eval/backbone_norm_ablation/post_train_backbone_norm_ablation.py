...
from dinoct.data import make_dataset
from dinoct.data.datasets import OCT
from dinoct.data.transforms import Ensure3CH, MaybeToTensor, PerImageZScore
from dinoct.eval import DEFAULT_ACC_TOLERANCES, curve_metrics_batch, metric_name_for_tolerance
from dinoct.utils import fix_random_seeds

from convnext_backbone_norm_ablation import LayerNorm as ConvNeXtLayerNorm

ORIG_H, ORIG_W = 512, 500
logger = logging.getLogger("dinoct")
BACKBONE_NORM_MODES = ("none", "final_norm_only", "all_norms")
...
def freeze_backbone_except_lora_and_norms(backbone: nn.Module, norm_mode: str = "final_norm_only"):
    ...
    if norm_mode_l == "none":
        return
    if norm_mode_l == "final_norm_only":
        final_norm = getattr(backbone, "norm", None)
        if final_norm is None:
            raise ValueError("Backbone has no `.norm` module; cannot use norm_mode='final_norm_only'.")
    ...
    if norm_mode_l == "all_norms":
        ...
        return
    raise ValueError(
        f"Unknown backbone norm mode {norm_mode!r}; expected one of {BACKBONE_NORM_MODES}."
    )
...
class CurveModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        backbone_norm_mode: str = "final_norm_only",
        backbone_checkpointing: bool = False,
    ):
        super().__init__()
        self.backbone = backbone
        if hasattr(backbone, "gradient_checkpointing"):
            setattr(backbone, "gradient_checkpointing", bool(backbone_checkpointing))
        ...
        freeze_backbone_except_lora_and_norms(backbone, norm_mode=backbone_norm_mode)
        ...
        self.backbone_norm_mode = str(backbone_norm_mode)
        self.backbone_checkpointing = bool(backbone_checkpointing)
...
def _post_train_split_indices(entries: np.ndarray, *, seed: int) -> tuple[list[int], list[int], str, int, int]:
    ...
    rng.shuffle(val_idx)
    return train_idx.tolist(), val_idx.tolist(), "fallback", 0, 0


class _OCTCodeSubset(Dataset):
    def __init__(self, base: OCT, indices: list[int]) -> None:
        self.base = base
...
def run_post_training(
    *,
    ...
    ds = _OCTCodeSubset(ds_full, train_idx)
    ...
    if len(dl) == 0:
        raise ValueError(
            ...
            "step loop would spin forever. Reduce batch_size or add data."
        )
    ...
    model = CurveModel(
        ...
        backbone_norm_mode=backbone_norm_mode,
        backbone_checkpointing=backbone_checkpointing,
    ).to(device)
    logger.info("post-train backbone norm mode: %s", backbone_norm_mode)
    logger.info("post-train backbone checkpointing: %s", bool(backbone_checkpointing))
