...
from dinoct.layers.drop_path import DropPath
...
class ConvNeXt(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # NOTE: non-native patch_size (e.g. 14) is supported via _maybe_resize_tokens, same as
        # dinoct/models/convnext.py — the old strict guard predated p14 support and broke the
        # norm ablation on the p14 backbone (2026-07-09).
    ...
    def _maybe_resize_tokens(self, feats: Tensor, *, input_hw: tuple[int, int]) -> Tensor:
        ...
        if (feats.shape[-2], feats.shape[-1]) == (target_h, target_w):
            return feats
        # ported from dinoct/models/convnext.py: resize to the configured token grid (p14 support)
...
def convnext_tiny_s2(*, patch_size: int | None = None, **kwargs) -> ConvNeXt:
    return ConvNeXt(patch_size=patch_size, stem_patch_size=2, stem_stride=2, **_CONVNEXT_SPECS["tiny"], **kwargs)


def convnext_small_s2(*, patch_size: int | None = None, **kwargs) -> ConvNeXt:
    return ConvNeXt(patch_size=patch_size, stem_patch_size=2, stem_stride=2, **_CONVNEXT_SPECS["small"], **kwargs)
