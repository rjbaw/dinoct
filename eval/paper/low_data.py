...
DEFAULT_FULL_TRAIN_REFS = {
    "DINOCT": {
        ...
        "real_hard_summary_path": REPO_ROOT / "outputs" / "paper_results" / "checkpoints" / "eval" / "real_hard" / "summary.json",
    ...
    "UNET": {
        ...
        "real_hard_summary_path": REPO_ROOT / "outputs" / "paper_results" / "checkpoints" / "unet" / "eval" / "real_hard" / "summary.json",
        "notes": "reused paper main checkpoint",
    },
    "FCBR": {
        ...
        "real_hard_summary_path": REPO_ROOT / "outputs" / "paper_results" / "checkpoints" / "fcbr" / "eval" / "real_hard" / "summary.json",
        "notes": "reused paper main checkpoint",
    },
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run low-data train+eval experiments for DINOCT, UNET, and FCBR, and write the data-efficiency manifest."
    )
    parser.add_argument(
        "--methods",
        nargs="+",
    ...
    parser.add_argument(
        ...
        help="matched-samples gives UNet/FCBR the same approximate number of post-train samples as DINOCT.",
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Optional CSV with columns: method,checkpoint_path,summary_path,"
            "real_hard_summary_path,notes. Used only for the 'full train set' row."
        ),
    )
...
def _update_manifest(
    *,
    ...
    budget_label: str,
    summary_path: Path,
    real_hard_summary_path: Path,
    notes: str,
) -> None:
    rows = _read_csv_rows(manifest_path) if manifest_path.exists() else []
    if not rows:
        rows = [
            {"method": method, "budget": budget, "summary_path": "", "real_hard_summary_path": "", "notes": ""}
    ...
    for row in rows:
        if str(row.get("method", "")).strip().upper() == method_label and str(row.get("budget", "")).strip() == budget_label:
            ...
            row["real_hard_summary_path"] = _rel(real_hard_summary_path)
            row["notes"] = notes
            updated = True
            break
    if not updated:
        rows.append(
            {
                ...
                "real_hard_summary_path": _rel(real_hard_summary_path),
                "notes": notes,
            }
        )
    for row in rows:
        row.setdefault("real_hard_summary_path", "")
    _write_csv_rows(manifest_path, ["method", "budget", "summary_path", "real_hard_summary_path", "notes"], rows)
    ...
    if path.exists():
        for row in _read_csv_rows(path):
            ...
            refs[method] = {
                ...
                "real_hard_summary_path": str(row.get("real_hard_summary_path", "")).strip(),
    ...
    for method, spec in DEFAULT_FULL_TRAIN_REFS.items():
        ...
        if ckpt.exists():
            refs[method] = {
                ...
                "real_hard_summary_path": _rel(Path(spec["real_hard_summary_path"]))
                if Path(spec["real_hard_summary_path"]).exists()
                else "",
...
def _baseline_steps_for_budget(*, model_type: str, batch_size: int, config_path: Path, args: argparse.Namespace) -> int | None:
    explicit = getattr(args, f"{model_type}_steps", None)
    if explicit is not None:
        return int(explicit)
    if args.steps is not None:
        return int(args.steps)
    if str(args.baseline_budget_mode) == "legacy-steps":
        return None

    cfg_source = config_path if config_path.exists() else args.config
    cfg = yaml.safe_load(cfg_source.read_text()) or {}
    post_cfg = cfg.get("post_train", {}) if isinstance(cfg, dict) else {}
    dinoct_steps = int(post_cfg.get("steps", 1500))
    dinoct_batch_size = int(args.dinoct_batch_size if args.dinoct_batch_size is not None else post_cfg.get("batch_size", 128))
    target_samples = int(dinoct_steps * dinoct_batch_size)
    return int((target_samples + max(int(batch_size), 1) - 1) // max(int(batch_size), 1))
...
def _train_baseline(*, model_type: str, run_dir: Path, config_path: Path, args: argparse.Namespace) -> Path:
    ...
    baseline_batch_size = _baseline_batch_size(model_type, args)
    baseline_steps = _baseline_steps_for_budget(
    ...
    cmd = [
        ...
        str(args.device),
        "--batch-size",
        str(baseline_batch_size),
        "--eval-every",
        str(args.baseline_eval_every),
    ]
    if baseline_steps is not None:
        cmd.extend(["--steps", str(baseline_steps)])
    _run(cmd, dry_run=args.dry_run)
    return ckpt
...
def _materialize_reference_full_train(
    *,
    ...
    real_hard_summary_ref = _resolve_reference_path(reference.get("real_hard_summary_path", ""))
    ...
    target_real_hard_eval_dir = run_dir / "eval" / "real_hard"
    target_real_hard_summary = target_real_hard_eval_dir / "summary.json"

    if ckpt_ref is not None and ckpt_ref.exists():
        eval_args = argparse.Namespace(**vars(args))
        eval_args.resume = False
        summary_path = _evaluate_curve(
            ckpt_path=ckpt_ref,
            config_path=config_path,
            output_dir=target_eval_dir,
            args=eval_args,
        )
        real_hard_summary_path = _evaluate_curve(
            ckpt_path=ckpt_ref,
            config_path=config_path,
            output_dir=target_real_hard_eval_dir,
            args=eval_args,
            eval_dir=args.real_hard_dir,
        )
        return summary_path, real_hard_summary_path, ckpt_ref, ref_notes

    if args.resume and target_summary.exists() and target_real_hard_summary.exists():
        return target_summary, target_real_hard_summary, ckpt_ref if ckpt_ref and ckpt_ref.exists() else None, ref_notes

    if summary_ref is not None and summary_ref.exists() and real_hard_summary_ref is not None and real_hard_summary_ref.exists():
        return summary_ref, real_hard_summary_ref, ckpt_ref if ckpt_ref and ckpt_ref.exists() else None, ref_notes

    raise FileNotFoundError(
        "Full-train reference for {} is invalid: checkpoint={}, clean_summary={}, real_hard_summary={}".format(
            method_label,
            ckpt_ref,
            summary_ref,
            real_hard_summary_ref,
        )
    )
...
def main() -> None:
    ...
    for budget_value, budget_label in budgets:
        ...
        for method in args.methods:
            ...
            real_hard_summary_path = run_dir / "eval" / "real_hard" / "summary.json"
            ...
            if budget_value is None and method_label in full_train_refs:
                summary_path, real_hard_summary_path, ckpt_path, extra_notes = _materialize_reference_full_train(
            ...
            elif args.resume and summary_path.exists() and real_hard_summary_path.exists():
                pass
            elif method == "dinoct":
                ...
                summary_path = _evaluate_curve(
                    ckpt_path=ckpt_path,
                    config_path=config_path,
                    output_dir=run_dir / "eval" / "clean_test",
                    args=args,
                )
                real_hard_summary_path = _evaluate_curve(
                    ckpt_path=ckpt_path,
                    config_path=config_path,
                    output_dir=run_dir / "eval" / "real_hard",
                    args=args,
                    eval_dir=args.real_hard_dir,
                )
            else:
            ...
            if not args.dry_run:
                _update_manifest(
                    ...
                    real_hard_summary_path=real_hard_summary_path,
                    notes=notes_full,
                )
                _append_run_record(
                    runs_csv,
