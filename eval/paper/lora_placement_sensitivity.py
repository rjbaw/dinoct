...
VARIANTS = [
    {
        "key": "no_lora",
        "label": "No LoRA",
        "change": r"$\mathrm{LoRA}=0$",
        "lora_blocks": 0,
    },
    {
        "key": "last_1_block",
        "label": "Last 1 block",
        "change": "LoRA on final ConvNeXt block",
        "lora_blocks": 1,
    },
    {
        "key": "last_3_blocks",
        "label": "Last 3 blocks",
        "change": "Reference placement",
        "lora_blocks": 3,
    },
    {
        "key": "last_6_blocks",
        "label": "Last 6 blocks",
        "change": "LoRA on final 6 ConvNeXt blocks",
        "lora_blocks": 6,
    },
    {
        "key": "last_12_blocks",
        "label": "Last 12 blocks",
        "change": "LoRA on final 12 ConvNeXt blocks (stages 3--4)",
        "lora_blocks": 12,
    },
    {
        "key": "all_18_blocks",
        "label": "All 18 blocks",
        "change": "LoRA on all ConvNeXt blocks",
        "lora_blocks": 18,
    },
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run LoRA-placement sensitivity analysis.")
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=REPO_ROOT / "outputs" / "paper_results" / "ablations" / "lora_placement_sensitivity",
    )
    parser.add_argument("--real-hard-dir", type=Path, default=REPO_ROOT / "data" / "oct" / "eval" / "hard")
    parser.add_argument(
        "--paper-table-path",
        type=Path,
        default=REPO_ROOT / "outputs" / "paper_tables" / "lora_placement_ablation.tex",
    )
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--seeds", type=int, nargs="+", default=None)
    parser.add_argument("--variants", nargs="+", choices=[str(item["key"]) for item in VARIANTS], default=None)
...
def run(cmd: list[str]) -> None:
    print("[lora-placement] running: {}".format(" ".join(cmd)))
    env = os.environ.copy()
...
def resolve_variants(args: argparse.Namespace) -> list[dict[str, Any]]:
    if not args.variants:
        return list(VARIANTS)
    selected = set(str(key) for key in args.variants)
    return [variant for variant in VARIANTS if str(variant["key"]) in selected]
...
def metadata_payload(args: argparse.Namespace, variant: dict[str, Any], seed: int) -> dict[str, Any]:
    lora_blocks = int(variant["lora_blocks"])
    return {
        "script": "eval/paper/lora_placement_sensitivity.py",
        ...
        "eval_overrides": ["--lora-blocks", str(lora_blocks)],
    }
...
def metrics_from_summary(path: Path, *, expected_lora_blocks: int) -> dict[str, float]:
    ...
    if isinstance(model_load, dict):
        ...
        actual_lora_blocks = model_load.get("lora_blocks")
        ...
        if actual_lora_blocks != expected_lora_blocks:
            raise ValueError(
                "LoRA block-count mismatch in {}: expected={} actual={}".format(
                    path,
                    expected_lora_blocks,
                    actual_lora_blocks,
                )
            )
...
def aggregate_variant_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    aggregated: dict[str, Any] = {
        ...
        "lora_blocks": int(rows[0]["lora_blocks"]),
        "num_seeds": len(rows),
...
def fmt(mean_value: float, std_value: float, *, bold: bool, multi_seed: bool, scale: float = 1.0) -> str:
    value = "{:.3f}".format(mean_value * scale)
    if bold:
        value = r"\textbf{" + value + "}"
    if not multi_seed:
        return value
    return value + r" $\pm$ " + "{:.3f}".format(std_value * scale)


def render_lora_table(rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        r"\begin{table*}[!t]",
        r"\centering",
        r"\caption{LoRA placement sensitivity analysis. Values are mean $\pm$ std across seeds when multiple seeds are provided. All variants keep the remaining training recipe fixed.}",
    ...
    lines.extend(
        [
            ...
            r"\end{tabularx}",
            r"\end{footnotesize}",
            r"\label{tab:lora_placement_ablation}",
            r"\end{table*}",
            "",
        ]
    )
...
def run_variant_seed(args: argparse.Namespace, variant: dict[str, Any], seed: int, multi_seed: bool) -> dict[str, Any]:
    ...
    clean = metrics_from_summary(clean_summary, expected_lora_blocks=lora_blocks)
    hard = metrics_from_summary(hard_summary, expected_lora_blocks=lora_blocks)
...
def main() -> None:
    ...
    per_seed_csv_path = args.output_root / "lora_placement_sensitivity_per_seed.csv"
    with per_seed_csv_path.open("w", newline="") as fh:
        writer = csv.DictWriter(fh, fieldnames=list(per_seed_rows[0].keys()))
        writer.writeheader()
        writer.writerows(per_seed_rows)

    aggregated_csv_path = args.output_root / "lora_placement_sensitivity.csv"
    with aggregated_csv_path.open("w", newline="") as fh:
        writer = csv.DictWriter(fh, fieldnames=list(aggregated_rows[0].keys()))
        writer.writeheader()
        writer.writerows(aggregated_rows)

    tex = render_lora_table(aggregated_rows)
    tex_path = args.output_root / "lora_placement_sensitivity.tex"
    ...
    print("[lora-placement] wrote {}".format(per_seed_csv_path))
    print("[lora-placement] wrote {}".format(aggregated_csv_path))
    print("[lora-placement] wrote {}".format(tex_path))
    if args.paper_table_path:
        print("[lora-placement] wrote {}".format(args.paper_table_path))
