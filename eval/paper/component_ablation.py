...
VARIANTS = [
    {
        'key': 'full',
        'label': 'Full',
        'change': 'Reference recipe; all backbone norms trainable',
        'overrides': [],
    },
    {
        'key': 'no_ema',
    ...
    {
        'key': 'no_backbone_norms',
        'label': '-BackboneNorms',
        'change': 'all backbone norms $\\rightarrow$ frozen',
        'overrides': ['--post-train-backbone-norm-mode', 'none'],
    },
]
...
def eval_marker_payload(variant: dict[str, Any]) -> dict[str, Any]:
    return {"eval_overrides": list(variant.get("eval_overrides", []))}


def eval_marker_matches(path: Path, variant: dict[str, Any]) -> bool:
    payload = eval_marker_payload(variant)
    if not payload["eval_overrides"]:
        return True
    if not path.exists():
        return False
    try:
...
def metadata_payload(args: argparse.Namespace, variant: dict[str, Any], seed: int) -> dict[str, Any]:
    return {
        "script": "eval/paper/component_ablation.py",
        "trainer": str(variant.get("train_script", "python -m dinoct")),
        ...
        "eval_overrides": list(variant.get("eval_overrides", [])),
    }
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        for seed in seeds:
            ...
            eval_marker = eval_dir / 'eval_overrides.json'
            metadata_path = variant_dir / 'run_metadata.json'
            ckpt = train_dir / 'post_train' / 'fused_curve_best.pth'
            train_ready = ckpt.exists() and metadata_matches(metadata_path, args, variant, seed)
            ...
            ready = (
                ...
                and eval_marker_matches(eval_marker, variant)
                and metadata_matches(metadata_path, args, variant, seed)
            )
            if not (args.resume and ready):
                if not (args.resume and train_ready):
                    train_script = variant.get('train_script')
                    if train_script is not None:
                        train_cmd = [
                            sys.executable,
                            str(REPO_ROOT / str(train_script)),
                            '--config', str(args.config),
                        ...
                        train_module = str(variant.get('train_module', 'dinoct'))
                        train_cmd = [
                            sys.executable,
                            '-m',
                            train_module,
                ...
                clean_cmd.extend(variant.get('eval_overrides', []))
                run(clean_cmd)
                ...
                hard_cmd.extend(variant.get('eval_overrides', []))
                run(hard_cmd)
                ...
                robust_cmd.extend(variant.get('eval_overrides', []))
                run(robust_cmd)

                eval_marker.write_text(json.dumps(eval_marker_payload(variant), indent=2) + '\n')
