...
class CurveDistributionHead(nn.Module):
    ...
    def __init__(self, in_channels: int, mid_channels: int = 128) -> None:
        ...
        self.out_z = nn.Conv2d(mid_channels, 1, kernel_size=1, bias=True)
    ...
    def forward(self, features: torch.Tensor, out_size_hw: tuple[int, int]) -> torch.Tensor:
        ...
        z_logits = F.interpolate(z_logits, size=(out_h, out_w), mode="bilinear", align_corners=False)
...
def decode_paper_unet_logits(
    ...
    *,
    ...
    argmax_z = epi.argmax(dim=1).float()
    z_hat = argmax_z.clone()
    ...
    z_hat = torch.where(present, midpoint, z_hat)
    ...
    return z_hat.to(seg_logits.dtype), presence_logits
