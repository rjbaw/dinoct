...
def _build_paper_seg_target(z_curve: np.ndarray | None, *, is_bg: bool, band_radius: int) -> np.ndarray:
    target = np.zeros((ORIG_H, ORIG_W), dtype=np.int64)
    if is_bg or z_curve is None:
        return target

    y_i = np.rint(np.asarray(z_curve, dtype=np.float32)).astype(np.int64)
...
def _curve_from_seg_target(seg_target: np.ndarray) -> np.ndarray:
    ...
    z[present] = 0.5 * (top[present].astype(np.float32) + bottom[present].astype(np.float32))
    return z
...
def _elastic_piecewise_affine(
    ...
    *,
    ...
    zz, xx = np.meshgrid(np.arange(h, dtype=np.float32), np.arange(w, dtype=np.float32), indexing="ij")
    points = np.stack([zz.reshape(-1), xx.reshape(-1)], axis=-1)
    interp_z = RegularGridInterpolator((grid_y, grid_x), disp_y, method="linear", bounds_error=False, fill_value=0.0)
    interp_x = RegularGridInterpolator((grid_y, grid_x), disp_x, method="linear", bounds_error=False, fill_value=0.0)
    flow_z = interp_z(points).reshape(h, w)
    flow_x = interp_x(points).reshape(h, w)

    coords = np.stack([zz + flow_z, xx + flow_x], axis=0)
...
class PaperUNetSegDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        z_curve = self.base.get_target(dataset_idx)
        is_bg = self.base.get_code(dataset_idx) == 2
        seg_target = _build_paper_seg_target(z_curve, is_bg=is_bg, band_radius=self.band_radius)
        ...
        z_t = torch.from_numpy(_curve_from_seg_target(seg_target))
        return {
            "image": image_t,
            "seg_target": seg_t,
            "z": z_t,
...
def _compute_seg_class_weights(dataset: OCT, indices: list[int], *, band_radius: int) -> torch.Tensor:
    ...
    for idx in indices:
        z_curve = dataset.get_target(int(idx))
        is_bg = dataset.get_code(int(idx)) == 2
        if is_bg:
            counts[0] += full_cols
            continue
        target = _build_paper_seg_target(z_curve, is_bg=False, band_radius=band_radius)
...
def _train_step_segmentation(
    *,
    ...
    with torch.no_grad():
        z_hat, presence_logits = decode_paper_unet_logits(seg_logits)
    ...
    *,
    ...
    with torch.inference_mode():
        for batch in val_loader:
            ...
            z_hat, _presence = decode_paper_unet_logits(seg_logits)
            non_bg = (is_bg == 0)
