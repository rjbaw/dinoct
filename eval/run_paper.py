...
def parse_args() -> argparse.Namespace:
    ...
    p = sub.add_parser(
        "robustness",
        help="Run the learned-model robustness suite for DINOCT, UNet, and FCBR into outputs/paper_results/robustness.",
    )
    _add_bundle_args(p)
    p.add_argument("--write-overlays", action="store_true")

    p = sub.add_parser("all", help="Run the full OCT paper pipeline: main checkpoints, robustness, ablations, and low-data.")
    _add_bundle_args(p)
    p.add_argument("--write-overlays", action="store_true")
...
def _run_ablations(resume: bool) -> None:
    cmd = [
        sys.executable,
        str(REPO_ROOT / "eval" / "paper" / "component_ablation.py"),
        "--output-root", str(PAPER_ABLATION_ROOT),
    ]
    if resume:
        cmd.append("--resume")
    _run(cmd)
...
def main() -> None:
    ...
    if args.command == "ablations":
        _run_ablations(resume=args.resume)
        return
