...
class ConvNeXt(nn.Module):
    ...
    - Optionally resizes the final feature map to match a configured patch-size token grid.
    """
    ...
    def __init__(
        ...
        *,
        ...
        n_blocks = int(sum(int(d) for d in depths))
        if drop_path_uniform:
            dp_rates = [float(drop_path_rate)] * n_blocks
        else:
            dp_rates = [float(x) for x in np.linspace(0, drop_path_rate, n_blocks)]
        self.drop_path_rates = tuple(dp_rates)
        cur = 0
