...
class BlockChunk(nn.ModuleList):
    def forward(
        ...
        for block in self:
            x_or_x_list = block(x_or_x_list, rope_or_rope_list)
        return x_or_x_list
...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        **ignored_kwargs,
        ...
        if len(ignored_kwargs) > 0:
            raise TypeError(f"Unsupported DinoVisionTransformer kwargs: {sorted(ignored_kwargs)}")
        del ignored_kwargs
        ...
        if drop_path_uniform:
            drop_path_rates = [float(drop_path_rate)] * depth
        else:
            drop_path_rates = [float(x) for x in torch.linspace(0, float(drop_path_rate), depth)]
        ...
        if block_chunks < 0:
            raise ValueError("block_chunks must be non-negative")
        if block_chunks > 0:
            if block_chunks > depth or depth % block_chunks != 0:
                raise ValueError("block_chunks must evenly divide depth")
            chunk_size = depth // block_chunks
            self.chunked_blocks = True
            self.blocks = nn.ModuleList(
                [BlockChunk(blocks_list[i : i + chunk_size]) for i in range(0, depth, chunk_size)]
            )
        else:
    ...
    def forward_features_list(self, x_list: list[Tensor], masks_list: list[Tensor | None]) -> list[dict[str, Tensor]]:
        if len(x_list) != len(masks_list):
            raise ValueError(
                f"Expected one mask per input crop, got {len(x_list)} inputs and {len(masks_list)} masks."
            )
        ...
        for idx, (x, masks) in enumerate(zip(all_x, masks_list)):
            if self.untie_cls_and_patch_norms or self.untie_global_and_local_cls_norm:
                if self.untie_global_and_local_cls_norm and self.training and idx >= n_global_crops:
    ...
    def _iter_blocks(self):
        for block_or_chunk in self.blocks:
            if isinstance(block_or_chunk, BlockChunk):
                yield from block_or_chunk
            else:
                yield block_or_chunk
    ...
    def _get_intermediate_layers(self, x: Tensor, n: int = 1) -> list[Tensor]:
        x, (H, W) = self.prepare_tokens_with_masks(x)

        output, flat_blocks = [], list(self._iter_blocks())
        total_block_len = len(flat_blocks)
        blocks_to_take = range(total_block_len - n, total_block_len) if isinstance(n, int) else n
        for i, blk in enumerate(flat_blocks):
            if self.rope_embed is not None:
    ...
    def forward(self, *args, is_training: bool = False, **kwargs) -> dict[str, Tensor] | list[dict[str, Tensor]] | Tensor:
        ret = self.forward_features(*args, **kwargs)
        if is_training:
            return ret
        if isinstance(ret, list):
            raise TypeError("DinoVisionTransformer.forward expects Tensor input when is_training=False.")
