...
def curve_metrics_batch(
    ...
    z_true: "torch.Tensor | np.ndarray",
    *,
    acc_tolerances: Iterable[float] = DEFAULT_ACC_TOLERANCES,
    spike_kappa: float | None = None,
) -> dict[str, "torch.Tensor | np.ndarray"]:
    if isinstance(z_pred, np.ndarray) or isinstance(z_true, np.ndarray):
        z_pred_f = np.asarray(z_pred, dtype=np.float32)
        z_true_f = np.asarray(z_true, dtype=np.float32)
        abs_err = np.abs(z_pred_f - z_true_f)
        bias = (z_pred_f - z_true_f).mean(axis=1)
        ...
        if spike_kappa is not None:
            if z_pred_f.shape[1] < 3:
                metrics_np["spike_rate"] = np.zeros(z_pred_f.shape[0], dtype=np.float32)
            else:
                d2 = z_pred_f[:, 2:] - 2.0 * z_pred_f[:, 1:-1] + z_pred_f[:, :-2]
    ...
    z_true_f = z_true.float()
    abs_err = (z_pred_f - z_true_f).abs()
    bias = (z_pred_f - z_true_f).mean(dim=1)
    ...
    if spike_kappa is not None:
        if z_pred_f.shape[1] < 3:
            metrics["spike_rate"] = torch.zeros(z_pred_f.shape[0], device=z_pred_f.device, dtype=z_pred_f.dtype)
        else:
            d2 = z_pred_f[:, 2:] - 2.0 * z_pred_f[:, 1:-1] + z_pred_f[:, :-2]
