...
def _without_backbone_prefix(name: str) -> str:
    # torch.compile wraps modules under "_orig_mod."; DDP/DataParallel add
    # "module.".  Strip all wrapper prefixes before matching architecture names.
    changed = True
    while changed:
        changed = False
...
def _is_embedding_or_stem_param(name: str) -> bool:
    return (
        any(k in name for k in ["pos_embed", "patch_embed", "mask_token", "cls_token", "storage_tokens"])
        or name.startswith("downsample_layers.0.")
        or name == "downsample_layers.0"
    )
...
def get_vit_lr_decay_rate(
    ...
    canonical_name = _without_backbone_prefix(name)
    if (
        force_is_backbone
        or canonical_name != name
        or name.startswith("backbone")
        or name.startswith("module.backbone")
        or name.startswith("_orig_mod.backbone")
    ):
        name = canonical_name
        if _is_embedding_or_stem_param(name):
            layer_id = 0
...
def get_params_groups_with_decay_fsdp(
    ...
    *,
    ...
    for name, param in model.named_parameters():
        ...
        if _is_embedding_or_stem_param(canonical_name):
            lr_mult *= patch_embed_lr_mult

        if wd_grouping == "legacy":
            # Legacy NAME-based no-decay ONLY: bias / LayerNorm.weight / norm / bn ->
            # layerscale-gamma + generic 1-D weights are DECAYED.
            lname = name.lower()
            no_decay = (
                name.endswith(".bias")
                or "layernorm.weight" in lname
                or "norm" in lname
                or ".bn" in lname
            )
        else:
