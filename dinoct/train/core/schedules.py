...
def linear_warmup_cosine_decay(
    *,
    ...
    cosine: list[float] = []
    if remain > 0:
        active_cosine_iters = min(remain, max(int(cosine_iters), 1))
        cosine = [
            end + 0.5 * (peak - end) * (1 + math.cos(math.pi * t / max(active_cosine_iters - 1, 1)))
            for t in range(active_cosine_iters)
        ]
        if active_cosine_iters < remain:
            cosine.extend([end] * (remain - active_cosine_iters))
