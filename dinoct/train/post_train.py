...
def confidence_weight(curve_logits_y: torch.Tensor, eps: float = 1e-12) -> torch.Tensor:
    """Return (B,W) normalized confidence weights in [0,1]."""
    p = F.softmax(curve_logits_y.float(), dim=1).clamp_min(float(eps))
...
def robust_curv_loss(curve_logits_y: torch.Tensor, non_bg_mask: torch.Tensor, delta: float = 1.0) -> torch.Tensor:
    ...
    confidence = confidence_weight(curve_logits_y)[:, 1:-1]  # align to W-2
...
def freeze_backbone_except_lora_and_norms(backbone: nn.Module):
    """Freeze backbone weights except LoRA adapters and all normalization layers."""
